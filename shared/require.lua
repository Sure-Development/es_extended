--[[
    _________________________________________
   |    __      _                            |
   |  o'')}____//      D O G E A T E R X     |
   |   `_/      )      Development           |
   |   (_(_/-(_/                             |
   |_________________________________________|

  Discord : https://discord.gg/Pnsta3xnZX

  ⚠  OUR CODE | THANKS FOR YOUR TRUSTED

  THIS IS A OX_LIB require EDITED
--]]

--[[
    https://github.com/overextended/ox_lib

    This file is licensed under LGPL-3.0 or higher <https://www.gnu.org/licenses/lgpl-3.0.en.html>

    Copyright © 2025 Linden <https://github.com/thelindat>
]]

local isClient = not IsDuplicityVersion()

local decors = {
  {
    search = '@onNet{name}',
    replace = function(name)
      return 'RegisterNetEvent(' .. name .. ')'
    end,
  },
  {
    search = '@on{name}',
    replace = function(name)
      return 'AddEventHandler(' .. name .. ')'
    end,
  },
  {
    search = '@command{name}',
    replace = function(name)
      return 'RegisterCommand(' .. name .. ')'
    end,
  },
  {
    search = '@onStart{cb}',
    replace = function(cb)
      local event = 'onClientResourceStart'
      if not isClient then
        event = 'onResourceStart'
      end

      return 'AddEventHandler(\'' .. event .. '\', ' .. cb .. ')'
    end,
  },
  {
    search = '@onStop{cb}',
    replace = function(cb)
      local event = 'onClientResourceStop'
      if not isClient then
        event = 'onResourceStop'
      end

      return 'AddEventHandler(\'' .. event .. '\', ' .. cb .. ')'
    end,
  },
  {
    search = '@emit{data}',
    replace = function(data)
      return 'TriggerEvent(' .. data .. ')'
    end,
  },
  {
    search = '@emitNet{data}',
    replace = function(data)
      if isClient then
        return 'TriggerServerEvent(' .. data .. ')'
      else
        return 'TriggerClientEvent(' .. data .. ')'
      end
    end,
  },
  {
    search = '@currentCoords',
    replace = function()
      return 'GetEntityCoords(cache?.ped or PlayerPedId())'
    end,
  },
  {
    search = '@currentHealth',
    replace = function()
      return 'GetEntityHealth(cache?.ped or PlayerPedId())'
    end,
  },
  {
    search = '@currentArmor',
    replace = function()
      return 'GetPedArmour(cache?.ped or PlayerPedId())'
    end,
  },
  {
    search = '@onCallback{data}',
    replace = function(data)
      return 'lib.callback.register(' .. data .. ')'
    end,
  },
  {
    search = '@emitCallback{data}',
    replace = function(data)
      return 'lib.callback.await(' .. data .. ')'
    end,
  },
}

local function escapePattern(s)
  return (s:gsub('([%^%$%(%)%%%.%[%]%*%+%-%?])', '%%%1'))
end

local function applyDecorators(content)
  for _, decor in ipairs(decors) do
    local pattern = escapePattern(decor.search)
    pattern = pattern:gsub('{.-}', function()
      return '%((.-)%)'
    end)

    content = content:gsub(pattern, function(name)
      return decor.replace(name)
    end)
  end

  return '_G._DECOR = true;' .. content
end

local loaded = {}

package = {
  path = './?.lua;./?/init.lua',
  preload = {},
  loaded = setmetatable({}, {
    __index = loaded,
    __newindex = noop,
    __metatable = false,
  }),
}

---@param modName string
---@return string
---@return string
local function getModuleInfo(modName)
  local resource = modName:match('^@(.-)/.+') --[[@as string?]]

  if resource then
    return resource, modName:sub(#resource + 3)
  end

  local idx = 4 -- call stack depth (kept slightly lower than expected depth "just in case")

  while true do
    local src = debug.getinfo(idx, 'S')?.source

    if not src then
      return cache.resource, modName
    end

    resource = src:match('^@@([^/]+)/.+')

    if resource and not src:find('^@@ox_lib/imports/require') then
      return resource, modName
    end

    idx += 1
  end
end

local tempData = {}

---@param name string
---@param path string
---@return string? filename
---@return string? errmsg
---@diagnostic disable-next-line: duplicate-set-field
function package.searchpath(name, path)
  local resource, modName = getModuleInfo(name:gsub('%.', '/'))
  local tried = {}

  for template in path:gmatch('[^;]+') do
    local fileName = template:gsub('^%./', ''):gsub('?', modName:gsub('%.', '/') or modName)
    local file = LoadResourceFile(resource, fileName)

    if file then
      tempData[1] = file
      tempData[2] = resource
      return fileName
    end

    tried[#tried + 1] = ('no file \'@%s/%s\''):format(resource, fileName)
  end

  return nil, table.concat(tried, '\n\t')
end

---Attempts to load a module at the given path relative to the resource root directory.\
---Returns a function to load the module chunk, or a string containing all tested paths.
---@param modName string
---@param env? table
local function loadModule(modName, env)
  local fileName, err = package.searchpath(modName, package.path)

  if fileName then
    local file = tempData[1]
    local resource = tempData[2]

    table.wipe(tempData)
    return assert(load(applyDecorators(file), ('@@%s/%s'):format(resource, fileName), 't', env or _ENV))
  end

  return nil, err or 'unknown error'
end

---@type PackageSearcher[]
package.searchers = {
  --[[function(modName)
    local ok, result = pcall(_require, modName)

    if ok then
      return result
    end

    return ok, result
  end,
  function(modName)
    if package.preload[modName] ~= nil then
      return package.preload[modName]
    end

    return nil, ('no field package.preload[\'%s\']'):format(modName)
  end,]]
  function(modName)
    return loadModule(modName)
  end,
}

---Loads the given module, returns any value returned by the seacher (`true` when `nil`).\
---Passing `@resourceName.modName` loads a module from a remote resource.
---@param modName string
---@return unknown
function require(modName)
  if type(modName) ~= 'string' then
    error(('module name must be a string (received \'%s\')'):format(modName), 3)
  end

  local module = loaded[modName]

  if module == '__loading' then
    error(('^1circular-dependency occurred when loading module \'%s\'^0'):format(modName), 2)
  end

  if module ~= nil then
    return module
  end

  loaded[modName] = '__loading'

  local err = {}

  for i = 1, #package.searchers do
    local result, errMsg = package.searchers[i](modName)

    if result then
      if type(result) == 'function' then
        result = result()
      end
      loaded[modName] = result or result == nil

      return loaded[modName]
    end

    err[#err + 1] = errMsg
  end

  error(('%s'):format(table.concat(err, '\n\t')))
end
